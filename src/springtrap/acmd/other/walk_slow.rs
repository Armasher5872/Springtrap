...
unsafe extern "C" fn springtrap_walk_slow_effect(agent: &mut L2CAgentBase) {
...
unsafe extern "C" fn springtrap_walk_slow_sound(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
    loop {
        frame(lua_state, 13.0);
        if is_excute(agent) {
...
unsafe extern "C" fn springtrap_walk_slow_expression(agent: &mut L2CAgentBase) {
    let lua_state = agent.lua_state_agent;
