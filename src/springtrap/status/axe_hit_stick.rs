...
unsafe extern "C" fn springtrap_axe_hit_stick_main_loop(weapon: &mut L2CWeaponCommon) -> L2CValue {
    ...
    let object_id = WorkModule::get_int(weapon.module_accessor, *WEAPON_SPRINGTRAP_AXE_INSTANCE_WORK_ID_INT_OBJECT_ID);
    ...
    if MotionModule::is_end(weapon.module_accessor) {
        if [hash40("hit_stick"), hash40("hit_stuck")].contains(&motion_kind) {
            MotionModule::change_motion(weapon.module_accessor, Hash40::new("hit_stuck"), 0.0, 1.0, false, 0.0, false, false);
        }
    }
    0.into()
}
...
unsafe extern "C" fn springtrap_axe_hit_stick_end_status(weapon: &mut L2CWeaponCommon) -> L2CValue {
    let object_id = WorkModule::get_int(weapon.module_accessor, *WEAPON_SPRINGTRAP_AXE_INSTANCE_WORK_ID_INT_OBJECT_ID);
    if WorkModule::is_flag(weapon.module_accessor, *WEAPON_SPRINGTRAP_AXE_INSTANCE_WORK_ID_FLAG_LINKED) {
        ...
        if object_id != *BATTLE_OBJECT_ID_INVALID {
            ...
            PostureModule::set_pos(weapon.module_accessor, &Vector3f{x: opponent_pos.x, y: opponent_pos.y+(7.0*opponent_scale), z: opponent_pos.z});
        }
        WorkModule::set_int(weapon.module_accessor, *BATTLE_OBJECT_ID_INVALID, *WEAPON_SPRINGTRAP_AXE_INSTANCE_WORK_ID_INT_OBJECT_ID);
        WorkModule::off_flag(weapon.module_accessor, *WEAPON_SPRINGTRAP_AXE_INSTANCE_WORK_ID_FLAG_LINKED);
    }
    0.into()
}
